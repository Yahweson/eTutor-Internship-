...
export default class ChangeDefaultPassword extends Component {
    ...
    onSubmit(e) {
        ...
        const lecturerPassword = this.state.password;
        const studentNo = 0;
        ...
        if (confirmed === lecturerPassword){
            this.setState({
                message:"Confirmed"
            });

            axios.post("http://localhost:5000/lecturerInfo/update/password/" + this.props.match.params.id, info, {
            }).then(res => {
                console.log(res)
            });

            window.location = '/LecturerDash/'+this.props.match.params.id;
        }else{
            this.setState({
