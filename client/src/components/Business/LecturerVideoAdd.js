...
export default class LecturerAssignmentAdd extends Component {
    ...
    onSubmit(e) {
        ...
        axios.post("http://localhost:5000/lecturerVideos/add", assignmentInfo, {
        }).then(res => {
            console.log(res)
        });

        formData.append('LecturerVideoFile', this.state.lecturerAssignmentFile);

        console.log(this.state.lecturerAssignmentFile);
        axios.post("http://localhost:5000/lecturerVideos/uploadAssignment", formData, {
        }).then(res => {
            console.log(res)
        });
       // window.location = '/';
